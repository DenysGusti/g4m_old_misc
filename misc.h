...
namespace g4m {
    ...
    public:
        ...
        void insertUpdate(IDX, VAL); // MG: 18.02.2022 Update existing element
        ...
        void clear(int i); // MG: 9 April 2018 erase element i
        ...
        IDX find_key(IDX i); //MG: 23.01.2020 find a key in the map
        VAL maxval(); //MG: 11.09.2021 find max value
        VAL maxRightVal(IDX i); //MG: 30.09.2021 find max value to the right of the current key val
        VAL maxRightVal(IDX i, IDX j); //MG: 28.03.2022 find max value between the keys i and j, i<=j
        VAL minRightVal(IDX i); //MG: 30.09.2021 find min value to the right of the current key val
        VAL minRightVal(IDX i, IDX j); //28.03.2022 find min value between the keys i and j, i<=j
        std::map<IDX, VAL> aMap; //MG: changed to make it compatible with the previous version (moved from private)
        bool nonZero(); // MG: 18.04.2016. Returns true if the map is fillied in with at least one non-zero value
    private:
...
//---------------------------------
    ...
    template<class IDX, class VAL>
    // MG: added for flexible manipulations (09.04.2018)
    void ipol<IDX, VAL>::clear(int i) {
        aMap.erase(i);
    }
    ...
    template<class IDX, class VAL>
    // MG: 18.02.2022 Update existing element
    void ipol<IDX, VAL>::insertUpdate(IDX i, VAL v) {
        if (aMap.size() > 0) {
    ...
    template<class IDX, class VAL>
    IDX ipol<IDX, VAL>::find_key(IDX i) {
        if (aMap.size() > 0) {
            if (aMap.find(i) != aMap.end()) {
                return (aMap.find(i)->first);
            } else { return (0); }
        } else {
            return (0);
        }
...
//    unsigned int regions = 0.5 + pow(2,i.size()); //MG: to make it compatible with VS
        ...
        for (iter = aMap.begin(); iter != aMap.end(); ++iter) {
            ...
            for (unsigned int j = 0; j < iter->first.size() && j < i.size(); ++j) {
                ...
                if (tmp > 0.) { pos += mul; }
                if (pos > regions - 1) {
                    std::cout << "misc.h:line 202: pos>regions-1: possible out of range problem!!!" << std::endl;
                    exit(1);
                }
    ...
    template<class IDX, class VAL>
    VAL ipol<IDX, VAL>::maxval() {//MG: 11.09.2021 find max value
        typename std::map<IDX, VAL>::iterator iter = aMap.begin();
        if (aMap.size() > 0) {
            VAL ret = iter->second;
            ++iter;
            while (iter != aMap.end()) {
                if (ret < iter->second) { ret = iter->second; }
    ...
    template<class IDX, class VAL>
    VAL ipol<IDX, VAL>::maxRightVal(IDX i) {//MG: 30.09.2021 find max value to the right of the current value
        typename std::map<IDX, VAL>::iterator iter = aMap.find(i);
        if (aMap.size() > 0 && iter != aMap.end()) {
            VAL ret = iter->second;
            ++iter;
            while (iter != aMap.end()) {
                if (ret < iter->second) { ret = iter->second; }
    ...
    template<class IDX, class VAL>
    VAL ipol<IDX, VAL>::maxRightVal(IDX i, IDX j) {//MG: 28.03.2022 find max value between the indexes i and j, i<=j
        typename std::map<IDX, VAL>::iterator iter = aMap.find(i);
        if (aMap.size() > 0 && iter != aMap.end()) {
            VAL ret = iter->second;
            ++iter;
            while (iter->first <= j && iter != aMap.end()) {
                if (ret < iter->second) { ret = iter->second; }
    ...
    template<class IDX, class VAL>
    VAL ipol<IDX, VAL>::minRightVal(IDX i) {//MG: 30.09.2021 find min value to the right of the current value
        typename std::map<IDX, VAL>::iterator iter = aMap.find(i);
        if (aMap.size() > 0 && iter != aMap.end()) {
            VAL ret = iter->second;
            ++iter;
    ...
    template<class IDX, class VAL>
    VAL ipol<IDX, VAL>::minRightVal(IDX i, IDX j) {//MG: 28.03.2022 find min value between indexes i and j
        typename std::map<IDX, VAL>::iterator iter = aMap.find(i);
        if (aMap.size() > 0 && iter != aMap.end()) {
            VAL ret = iter->second;
            ++iter;
    ...
    private:
        ...
        int fill(ipol<std::vector<double>, VAL> &, unsigned int idx, unsigned int adim, unsigned int mul,
    ...
    template<class VAL>
    int fipolm<VAL>::fill(ipol<std::vector<double>, VAL> &t, unsigned int idx, unsigned int adim, unsigned int mul,
                          std::vector<double> key) {
        key.push_back(0.);
    ...
    private:
        ...
        int fill(ipol<std::vector<double>, VAL> &, unsigned int idx, unsigned int adim, unsigned int mul,
                 std::vector<double> key, double add);
    };
    ...
    template<class VAL>
    int ffipolm<VAL>::fill(ipol<std::vector<double>, VAL> &t, unsigned int idx, unsigned int adim, unsigned int mul,
                           std::vector<double> key, double add) {
        key.push_back(0.);
        for (unsigned int i = 0; i < n[adim]; ++i) {
